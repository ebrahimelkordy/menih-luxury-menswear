...
export default function AdminProcessPage() {
  const [items, setItems] = useState<ProcessStage[]>([]);
  ...
  const load = async () => {
    setLoading(true);
    const { data } = await supabase.from('process_stages').select('*').order('sort_order', { ascending: true });
    setItems(data ?? []);
    setLoading(false);
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Delete this process stage?')) return;
    await supabase.from('process_stages').delete().eq('id', id);
    load();
  };

  const toggleEnabled = async (item: ProcessStage) => {
    await supabase.from('process_stages').update({ enabled: !item.enabled }).eq('id', item.id);
    load();
  };

  if (loading) return <LoadingState label="LOADING PROCESS STAGES" />;
    ...
    <div>
      <div className="flex items-center justify-between mb-10">
        <div>
          ...
          <h1 className="font-display text-3xl md:text-4xl font-bold tracking-tightest text-graphite">PROCESS</h1>
        </div>
      ...
      {showForm && (
        <ProcessForm item={editing} onClose={() => { setShowForm(false); setEditing(null); load(); }} />
      )}

      {items.length === 0 && !showForm ? (
        <EmptyState title="No process stages yet" description="Add the stages of your build process." action={<AdminButton onClick={() => { setEditing(null); setShowForm(true); }}><Plus className="w-4 h-4" /> ADD STAGE</AdminButton>} />
      ) : (
...
function ProcessForm({ item, onClose }: { item: ProcessStage | null; onClose: () => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    if (item) {
      await supabase.from('process_stages').update({ ...form, updated_at: new Date().toISOString() }).eq('id', item.id);
    } else {
      await supabase.from('process_stages').insert(form);
    }
    setSaving(false);
    onClose();
  };
    ...
    <div className="bg-white border border-foreground/10 p-6 mb-6 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="inline-block w-1.5 h-1.5 bg-safety" />
          <span className="tech-label-sm text-foreground/40">{item ? 'EDIT' : 'NEW'} STAGE</span>
        </div>
