...
import {
  ...
  Layers, Package, Eye, Image as ImageIcon, CheckCircle, Tag, Upload
} from 'lucide-react';
    ...
    setFormData((prev) => ({
      ...
      variants: [
        ...prev.variants,
        {
          ...
          colorHex: '#1A1615',
          colorFamily: 'bold',
          image: heroImages.thobeGandoura,
          inStock: true,
        },
      ],
    }));
    ...
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filteredProducts.map((product) => {
          ...
          const mainVariant = variantsList[0];
          return (
            <div
              key={product.id}
            ...
            >
              <div className="relative aspect-[4/3] bg-espresso overflow-hidden">
                ...
                <div className="absolute bottom-2 right-2 left-2 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-ivory/70">{variantsList.length} ?????</span>
                </div>
              </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-espresso/85 backdrop-blur-md p-3 sm:p-6 md:p-8 flex items-start justify-center animate-fade-in">
          <div className="relative w-full max-w-3xl bg-espresso-light border border-terracotta/40 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden my-4 sm:my-8 animate-scale-up">
            {/* Header */}
            <div className="bg-espresso border-b border-ivory/10 px-5 sm:px-6 py-4 flex items-center justify-between">
              <h3 className="font-serif text-base sm:text-lg font-bold text-ivory flex items-center gap-2">
                <Package className="w-5 h-5 text-terracotta flex-shrink-0" />
            ...
            <form onSubmit={handleSaveProduct} className="p-4 sm:p-6 space-y-5 text-xs">
                {/* Titles & Category */}
                        ...
                        <div className="flex items-center gap-2">
                          {variant.image && (
                            <img
                              src={variant.image}
                              alt="Variant"
                              className="w-8 h-8 rounded-lg object-cover border border-ivory/20 flex-shrink-0"
                            />
                          )}
                          <input
                            type="text"
                            ...
                            placeholder="https://..."
                            className="w-full px-2.5 py-1.5 bg-espresso border border-ivory/10 rounded-lg text-ivory font-mono text-[11px]"
                            dir="ltr"
                          />
                          <label className="p-1.5 bg-terracotta/20 hover:bg-terracotta/40 text-terracotta border border-terracotta/30 rounded-lg transition-colors cursor-pointer flex-shrink-0" title="??? ???? ?? ??????">
